...
function LessonViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<LessonInterface>(
    ...
    () =>
      getLessonById(id, {
        relations: ['organization', 'assignment'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('lesson', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/lessons/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/assignments/create?lesson_id=${data?.id}`}>
